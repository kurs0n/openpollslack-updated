...
export const btn_vote8 = (app,mutexes,votesCol,getInfos,buildInfosBlocks,buildMenu)=>{
    app.action('btn_vote8', async ({ action, ack, body, context }) => {
        await ack();
          ...
          try {
            ...
            for (const question in poll) {
                poll[question].forEach(async item => {
                  if (item.userId === user_id && item.points === 8 && question != value.id) {
                ...
                if (alreadyVoted) {
                  await app.client.chat.postEphemeral({
                      ...
                      user: body.user.id,
                      attachments: [],
                      text: "You can't vote anymore for 8 points. Remove a vote to choose another option.",
                  });
                  release();
                  return;
              }
            ...
            if (removeVote) {
              ...
              await app.client.chat.postEphemeral({
                ...
                user: body.user.id,
                attachments: [],
                text: `You've vote for 8 points on option ${question}`,
            });
            }
